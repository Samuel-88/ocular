defmodule Ocular.Accounts do
  ...
  alias Ocular.Accounts.{Player, PlayerToken, PlayerNotifier}
  ...
  def render_player_event(build_id, player) do
    build = Repo.get!(Build, build_id)
    IO.inspect(player, limit: :infinity)
    IO.inspect(player.id, label: "PLAYER ID")
    IO.inspect(build, label: "BUILD")

    cond do
      build.player_id == player.id ->
        html = """
        <button type="button" class="btn btn-dark" onclick="location.href='/builds/#{build_id}/leave_build'">Leave spot!</button>
        """

        Phoenix.HTML.raw(html)

      build.player_id ->
        player = Repo.get!(Player, build.player_id)

        player.name

      true ->
        html = """
        <button type="button" class="btn btn-dark" onclick="location.href='/builds/#{build_id}'">Fill spot!</button>
        """

        Phoenix.HTML.raw(html)
    end

    # if build.player_id do
    #   player = Repo.get!(Player, build.player_id)
